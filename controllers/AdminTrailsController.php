...
class AdminTrailsController extends Controller {
    ...
    public function manageTrails() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['trail_id']) && !empty($_POST['trail_id'])) {
...
public function createTrails() {
    ...
    if ($isEdit) {
        ...
        if ($trail) {
            // Pré-remplir les champs avec les données récupérées, y compris les coordonnées
            $trailsData = [
                ...
                'status' => $trail['status'] ?? '',
                'longitude' => $trail['longitude'] ?? '', // Ajouter longitude
                'latitude' => $trail['latitude'] ?? ''    // Ajouter latitude
            ];
        } else {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $status = $_POST['status'] ?? '';
        $longitude = $_POST['longitude'] ?? '';  // Champ longitude
        $latitude = $_POST['latitude'] ?? '';    // Champ latitude
        $errors = [];
        ...
        if (empty($errors)) {
            if ($isEdit) {
                // Mise à jour du sentier avec les nouvelles coordonnées
                ...
                // Création d'un nouveau sentier avec les coordonnées
